...
In [Lab 2](../Lab02-First-MCP-Server/), you created a basic `MCP` server with a simple "hello world" tool. Now it's time to level up! 
...
- Completed [Lab 2 - Building Your First MCP Server](../Lab02-First-MCP-Server/)
...
2. **Self-Descriptive**: Clear name and description
3. **Well-Validated**: Comprehensive input schema
4. **Error-Resilient**: Handles failures gracefully
5. **Efficient**: Returns results quickly
6. **Composable**: Can work with other tools
...
The server will run on [http://localhost:11434](http://localhost:11434) by default.

**Step 4: Test the Weather Tool**

1. **Start the MCP Inspector** (in a new terminal):
   ```bash
...
**Troubleshooting:**
  ...
  - If you get "Ollama API error", make sure `ollama serve` is running
  - If you see fallback data, Ollama might not be responding properly
  - Check that the model is downloaded with `ollama list`
  - Try a different model if gpt-oss:20b doesn't work well
...
In [**Lab 4**](../Lab04-MCP-Resources/), you'll explore MCP Resources. You'll learn:
...
**You're becoming an MCP expert! Continue to [**Lab 4**](../Lab04-MCP-Resources/).**
